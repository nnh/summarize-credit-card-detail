const csvSaveFolder = DriveApp.getFolderById(PropertiesService.getScriptProperties().getProperty("csvSaveFolderId"));
...
function getTargetYears_(targetYear){
  const result = [4, 5, 6, 7, 8, 9, 10, 11, 12].map(month => `'${targetYear}年${month}月`).concat([1, 2, 3].map(month => `'${Number(targetYear) + 1}年${month}月`));
  return(result);
}
function onOpen() {
