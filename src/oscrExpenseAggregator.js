function oscrExpenseAggregator() {
  ...
  const targetYears = currentMonth > 3 ? [String(currentYear - 1), String(currentYear)] : [String(currentYear - 2), String(currentYear - 1)];
  // 数字4桁のシート名を取得
  const inputSheetNames = outputSpreadSheet.getSheets().map(x => x.getName())
    .filter(x => /^\d{4}$/.test(x) && targetYears.includes(x));
  if (inputSheetNames.length === 0){
    return;
  }
  inputSheetNames.forEach(sheetName => {
    const sheet1 = createSheetForOscrExpenseAggregator_(outputSpreadSheet.getSheetByName(sheetName), `${sheetName}_1`);
    sheet1.getRange(outputSheetNumber.get("headerRow"), outputSheetNumber.get("sumColumn"), sheet1.getLastRow(), 1)
      .setBorder(null, true, null, true, null, null);
    sheet1.hideColumns(outputSheetNumber.get("monthlyAverageColumn"), sheet1.getLastColumn() - outputSheetNumber.get("monthlyAverageColumn") + 1);
    const sheet2 = createSheetForOscrExpenseAggregator_(outputSpreadSheet.getSheetByName(sheetName), `${sheetName}_2`);
...
function copyValuesAndColumnWidths_(inputSheet, outputSheet) {
  ...
  const hideRowIndex = outputSheet.getRange(1, outputSheetNumber.get("itemColumn"), outputSheet.getLastRow(), 1).getValues().
    map((value, idx) => value[0] === validationSumText ? idx: null).filter(x => x !== null);
