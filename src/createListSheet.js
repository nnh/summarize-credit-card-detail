...
function createListSheet_(targetYear) {
  const itemsSheet = outputSpreadSheet.getSheetByName(
    inputSheetNames.get('itemsSheet')
  );
  if (itemsSheet === null) {
  ...
  const targetItemsValue = itemsValue.filter(
    (values, idx) =>
      idx === 0 ||
      (values[itemSheetIndex.get('startYear')] <= Number(targetYear) &&
        Number(targetYear) <= values[itemSheetIndex.get('endYear')])
  );
  const sortItemsValue = sortMatrix_(
    targetItemsValue,
    itemSheetIndex.get('sortOrder')
  );
  const outputItems = sortItemsValue
    .filter((_, idx) => idx > 0)
    .map(x => [x[itemSheetIndex.get('name')]]);
  const outputTargetYearColumns = getTargetYears_(targetYear);
  const outputColumns = [
    [
      ...outputTargetYearColumns,
      sumText,
      '月平均',
      '前年度月平均',
      '差額',
  ...
  const monthCols = Array(12)
    .fill()
    .map((_, idx) => idx + 2);
  const outputFormulas = outputItems.map((_, idx) => {
    const yearsFormulas = monthCols.map(
      col =>
  ...
  targetSheet
    .getRange(
      outputSheetNumber.get('bodyStartRow'),
      outputSheetNumber.get('itemColumn'),
      outputItems.length,
      1
    )
    .setValues(outputItems);
  const outputValues = [...outputColumns, ...outputFormulas];
  targetSheet
    .getRange(
      outputSheetNumber.get('headerRow'),
      outputSheetNumber.get('yearStartColumn'),
      outputValues.length,
      outputValues[0].length
    )
    .setValues(outputValues);
  const sumValues = monthCols.map(
    col =>
      `=sum(indirect("R2C${col}", false):indirect("R${outputValues.length - 1}C${col}", false)) - indirect("R${outputValues.length}C${col}", false)`
  );
  ...
  targetSheet
    ...
    .setValues([[validationSumText, ...sumValues]]);
...
function setSheetFormat_(targetSheet) {
  ...
  targetSheet.setColumnWidths(
    outputSheetNumber.get('yearStartColumn'),
    outputSheetNumber.get('categoryColumn') -
      outputSheetNumber.get('yearStartColumn'),
    85
  );
  targetSheet.setColumnWidth(
    outputSheetNumber.get('monthlyAverageLastYearColumn'),
    95
  );
